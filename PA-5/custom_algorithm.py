...
def find_max_sets(tasks):
    ...
    for schedule in all_schedules:
        ...
        if not any(set((task.pay, task.start, task.end, task.name) for task in other_schedule).issubset(schedule_set) for other_schedule in all_schedules if other_schedule != schedule):
